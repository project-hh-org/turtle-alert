...
async function initDetector() {
  ...
  try {
    // 메타 패키지(@tensorflow/tfjs) 는 webgl/webgpu/layers/data 백엔드까지 정적
    // require 해서 번들을 ~200MB 부풀린다. Node CPU 경로만 쓰는 우리 앱은
    // core + converter + cpu 백엔드만 직접 로드한다. (MoveNet 이 요구하는 최소셋)
