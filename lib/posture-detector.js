...
//
// false positive 0 정책 (v0.7.6+): 정상 자세에서 절대 알림이 뜨지 않도록
// 임계값을 보수적으로 잡는다. 진짜 거북목은 강 단독 신호로도 잡히므로
// 실제 검출률 저하는 작다.
//
// 트리:
...
const BASELINE_DELTA = {
  ...
  // 한쪽으로 기울어짐 증가량
  noseOffsetIncrease: 0.06,
  // 고개 기울어짐 증가량
  headTiltIncrease: 0.05,
  // 구부정 (약한 단독 신호)
  noseShoulderSlouch: 0.08,
};

// 카메라가 이동/거리가 바뀐 것으로 의심되는 임계 — baseline 의 어깨 너비 대비
// 현재 어깨 너비가 이 비율 이상 다르면 baseline 무효로 간주.
const CAMERA_SHIFT_RATIO = 0.30;
...
// 연속 나쁜 자세 감지 횟수 — false positive 0 정책상 한 번 더 확인 후 알림
...
/**
 * 현재 측정값의 어깨 너비가 baseline 의 어깨 너비와 크게 달라졌는지 확인.
 * 크게 다르면 카메라 위치/거리가 변한 것으로 간주 — baseline 비교는 의미가 없다.
 *
 * @returns {boolean} 카메라가 옮겨진 것 같으면 true
 */
function isCameraShifted(currentMetrics, baseline) {
  if (!currentMetrics || !baseline || !baseline.metrics) return false;
  const base = baseline.metrics.shoulderWidth;
  const now = currentMetrics.shoulderWidth;
  if (!base || !now) return false;
  return Math.abs(now - base) / base > CAMERA_SHIFT_RATIO;
}
